 ...
 *
 * @author mariano
 * @author mateo -> used the Java 5 functionality to stop the warnings.
 */
public class Interpreter {
    ...
    private Hashtable<Integer, String> jumpLabels;
    private Hashtable<Integer, Integer> opcodes;
    ...
    public Interpreter(String program, Machine machine){
        ...
        jumpLabels = new Hashtable<Integer, String>();
        opcodes = new Hashtable<Integer, Integer>();
        parseLabels();
    ...
    private void jumpToLabel(String label) throws Exception
    {
        if(labels.containsKey(label))
        {
            currentLine = (labels.get(label)).getLine();
        }
        else
        {
    ...
    public int getOpcode(int line) {
        if(opcodes.containsKey(line)) {
            if(jumpLabels.containsKey(line)) {
                lastLabel = (String)jumpLabels.get(line);
                inJump = true;
            }
            return opcodes.get(line);
        }
    ...
    public void setOpcode(int line, int opcode) {
        opcodes.put(line, opcode);
    }

    public void setCurrentOpcode(int opcode) {
        opcodes.put(getCurrentLine(), opcode);
    }
